...
class PropSeqDataset(EDVCdataset):
    ...
    def load_feats(self, key, path=None):
        ...
        if type(vf_types) == list:
            ...
            for vf_type, vf_folder in zip(vf_types, self.feature_folder):
                feats, is_padding = get_feats(key, vf_type, vf_folder, path=path)
    ...
    def __getitem__(self, idx):
        ...
        path = self.anno[key].get("path", None)
        feats = self.load_feats(key, path)
        ...
        if self.opt.caption_decoder_type != 'gpt2':
            caption_label = [np.array(self.translator.translate(sent, self.max_caption_len)) for sent in captions]
        else:
            caption_label = [np.array(self.translator(sent, max_length=self.max_caption_len, truncation=True)['input_ids']) for sent in captions]
...
def get_feats(key, vf_type, vf_folder, data_norm=False, path=None):
    MEAN = VAR = 0
    if vf_type == 'c3d':
        feat_dim = 500
        ...
        feat_dim = 128
        path = os.path.join(vf_folder, key[0:13] + '.npy')
    elif vf_type == 'clip' or vf_type == 'clippostnorm':
        feat_dim = 768
#        path = os.path.join(vf_folder, key[2:13] + '.npy' if not path else path)
#        path = os.path.join(vf_folder, key[:-6] +  'clip.npy' if not path else path)
        path = os.path.join(vf_folder, key +  '.npy' if not path else path)
#        path = os.path.join(vf_folder, key +  '_clip.npy' if not path else path)
    elif vf_type == 'clippreproj':
        feat_dim = 1024
